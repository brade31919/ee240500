...
int leftrotate(float push,float x,float y,float z,float the);
int rightrotate(float push,float x,float y,float z,float the);
double speedestimate(float push,float x,float y,float z,float the,double v);
int flash(float x,float the);
	...
	while (1) {
		...
		if (xdata_fp = fopen(filename, "w")) {
			...
			for (i=0; i<sample; i++) {
				...
				#ifdef DEBUG
				left = leftrotate(data_TSI,data_x,data_y,data_z,theta);
				right = rightrotate(data_TSI,data_x,data_y,data_z,theta);
				velocity= speedestimate(data_TSI,data_x,data_y,data_z,theta,velocity);
				//velocity += speedup*speedratio;
                f = flash(data_x,theta);
				
				printf("X command receive: (%d %d %g %d) t=%d\n", left, right, velocity,f,timecycle);
				#endif

				if (i != sample-1) {
					fprintf(xdata_fp, "[%d, %d, %g, %d],\n", left, right, velocity,f);
...
int leftrotate(float push,float x,float y,float z,float the){
	if(y>1000 && the>5 && the<20)
	{
		return 1;
	}
    else if(y>1000 && the>=20 && the<30)
    {
        return 2;
    }
    else if(y>1000 && the>=30 && the<40)
    {
        return 3;
    }
    else if(y>1000 && the>=40)
    {
        return 4;
    }
